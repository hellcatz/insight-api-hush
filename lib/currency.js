...
function CurrencyController(options) {
  ...
  this.poloniexRate = 0; // BTC/HUSH is not on poloniex, uses coinmarket cap https://api.coinmarketcap.com/v1/ticker/hush/
...
CurrencyController.prototype.index = function(req, res) {
  ...
  if (self.bitstampRate === 0 || currentTime >= (self.timestamp + self.currencyDelay)) {
    ...
    request('https://www.bitstamp.net/api/ticker/', function(err, response, body) {
      ...
      request('https://api.coinmarketcap.com/v1/ticker/hush/', function (err, response, body) {
        if (err) {
          self.node.log.error(err);
        }
        if (!err && response.statusCode === 200) {
